...
function renderSetDetail(setData, cards) {
    const isMcqSet = setData.cardType === 'mcq' || cards.some(card => Array.isArray(card.options) && card.options.length > 0);
    const content = `
        <!-- Hero Section -->
        ...
        <div class="container mx-auto px-4 max-w-6xl pb-12">
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                <div class="px-8 py-6 border-b border-gray-100 bg-gray-50">
                    <h2 class="text-2xl font-bold text-gray-900">${isMcqSet ? 'Questions in this set' : 'Terms in this set'} (${cards.length})</h2>
                </div>
                            ...
                            <div class="p-6 hover:bg-gray-50 transition-colors">
                                <div class="flex items-start space-x-6">
                                    ...
                                    <div class="flex-1 grid md:grid-cols-2 gap-6">
                                        <div>
                                            <div class="text-xs uppercase font-semibold text-gray-500 mb-2 tracking-wide">${isMcqSet ? 'Question' : 'Term'}</div>
                                            <p class="text-gray-900 font-medium text-lg">${isMcqSet ? (card.prompt || card.definition || '') : card.term}</p>
                                        </div>
                                        <div class="border-l border-gray-200 pl-6">
                                            <div class="text-xs uppercase font-semibold text-gray-500 mb-2 tracking-wide">${isMcqSet ? 'Correct Answer' : 'Definition'}</div>
                                            <p class="text-gray-700 text-lg">${isMcqSet ? (card.correctAnswer || card.term || '') : card.definition}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
